...
import sys
sys.path.insert(0, '/home/goodnight1/files/')
...
source_diri = {'loca2':'/home/goodnight1/loca2/NCA5/', 'star':'/home/goodnight1/star/NCA5/'}
...
for method in list(source_diri.keys()):
    ...
    for fname in os.listdir(diri):
        if fname.startswith('.') or ('mask' in fname):
            pass
        else:
            var   = fname.split('.')[0]
            #print(var)
...
def temp_conversion(da, fname): 
    ...
    if ('txx' in fname) or ('tasmax' in fname) or ('tasmean' in fname) or ('tnn' in fname) or ('tasmin' in fname):
        if ('_ge_' not in fname) & ('_le_' not in fname) & ('PRISM' not in fname) & ('STAR' not in fname):
            da -= 273.15
            return da
        else:
            return da 
...
def temp_conversion2(da, fname): 
    ...
    if ('txx' in fname) or ('tasmax' in fname) or ('tasmean' in fname) or ('tnn' in fname) or ('tasmin' in fname) or ('TMAX' in fname) or ('TMIN' in fname) or ('TMEAN' in fname):
        if ('_ge_' not in fname) & ('_le_' not in fname) & ('PRISM' in fname):
            da += 273.15
            return da
        else:
            return da 
...
for mask in region_dict.values():
    data = getData(source_diri['loca2'] + fname, 'annual', mask, qvar = None)
...
data = getData(source_diri[method] + method_files[method][key], 'annual', us_mask)
lat_name, lon_name = getLatLonNames(data)
    ...
    def update(click, method, model, member, region, season, variable, slider, previous_selection): 
        ...
        if key not in list(method_files[method].keys()):
            ...
            if triggered_input == 'usmap': # if user selected a region, get the regional data
                ...
                dataArr = getData(source_diri[method] + method_files[method][key], season_ind, region_dict[region], qvar = qvar)
                ...
                if (triggered_input == 'regionSelect') or (previous_selection == 'usmap') or (triggered_input is None) or (previous_selection is None): # US view
                    ...
                    dataArr   = getData(source_diri[method] + method_files[method][key], season_ind, region_dict[subregion], qvar = qvar).isel(region = 0)
                    stored_value = 'usmap'
                else: # subregion view
                    dataArr = getData(source_diri[method] + method_files[method][key], season_ind, region_dict[region], qvar = qvar)
                    lat_name, lon_name = getLatLonNames(dataArr)
